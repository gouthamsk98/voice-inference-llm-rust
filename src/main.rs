use candle_core::{ Device, Result, Tensor, DType, Error, D };
use candle_nn::{ Linear, Module, VarBuilder, VarMap, ops, loss, Optimizer };
...
fn train(m: Dataset, dev: &Device) -> Result<MultiLevelPerceptron> {
    ...
    if final_accuracy < 100.0 {
        Err(Error::Msg("The model is not trained well enough.".to_string()))
    } else {
        Ok(model)
    }
}
...
fn main() -> Result<()> {
    ...
    let train_results_tensor = Tensor::from_vec(
        train_results_vec,
        train_votes_vec.len() / VOTE_DIM,
        &dev
    )?;
    ...
    let test_results_tensor = Tensor::from_vec(
        test_results_vec.clone(),
        test_results_vec.len(),
        &dev
    )?;
    ...
    let tensor_test_votes = Tensor::from_vec(
